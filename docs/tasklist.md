...
| [ ] | EXT-006 | P2 | L | M3 | IMP-024 | 外部アクション（メール送信）連携 | 送信API（SES等）を選定し、承認後に実行できる |
...
- 対象: **MS=M0 の未完了タスク（P0/P1）**。現状は P0 のみ。
- 対象外: **M1/M2/M3 の全タスク（P0/P1/P2 含む）**。P1タスクは全て M2 以降のため今回対象外。
