...
#### ログイン画面の emotion 使い分け（Issue #34）

ログイン画面では「ハイサイくん（サイハイくん）」の表情を、画面状態の要約として次のルールで切り替えます（上ほど優先）。

| 優先 | 状態（ログイン画面） | emotion | 意図 |
|---:|---|---|---|
| 1 | 失敗/エラー表示中（未入力・認証失敗・タイムアウト等） | `anxiety` | 入力/操作の見直しを促す |
| 2 | ログイン実行中（認証リクエスト送信〜応答待ち） | `energy` | “動いている”を伝える |
| 3 | 入力完了（User ID と Password が両方入力済み） | `hope` | “次の一手”の状態を表す |
| 4 | 入力中（どちらか一方のみ入力済み） | `effort` | 入力継続を後押しする |
| 5 | 初期表示（未入力） | `standard` | 待機状態（ニュートラル） |

補足:
- 成功時は通常 `/dashboard` へ遷移するため、ログイン画面上の専用表情は基本不要。もし “成功表示” を入れる場合は `relief` を推奨。
- 画像が存在しない emotion が指定された場合は `standard` にフォールバックする（`frontend/src/app/core/haisa-emotion.ts`）。
