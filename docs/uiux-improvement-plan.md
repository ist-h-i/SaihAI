...
- ハイサイくんの画像表情と発言内容の整合性を上げ、感情表現の一貫性（体系/ルール/実装）を作る。
...
- ハイサイくんの発話 UI は `app-haisa-speech`（`frontend/src/app/components/haisa-speech.component.ts`）。
...
## ハイサイくん整合（emotion 体系・割当ルール・アセット整理）
...
- ハイサイくんの emotion 定義を 1 箇所に集約（重複解消の着手）。
...
- ハイサイくん: emotion 定義を `frontend/src/app/core/haisa-emotion.ts` に集約し、アセットを `frontend/src/assets/haisaikun` に統一。
...
- ハイサイくんの表情体系（emotion 一覧）と割当ルール（tone/状態/イベント→emotion）、整理方針（重複解消/アセット統一）がある。
