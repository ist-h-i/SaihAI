...
- **機能カテゴリと要件:**
  ...
  - **プロジェクト・メンバー可視化**
    ...
    - メンバーの「Genome（ゲノム）」詳細（スキルレベル、モチベーション推移、キャリア志向）を表示する。
...
- **機能カテゴリと要件:**
  ...
  - **データ投入バッチ**
    - 週報などの入力ソースを `weekly_reports` に取り込み、必要に応じて埋め込み更新を行う。
  - **Genome 解析・RAG 基盤**
    - pgvector に週報やキャリア志向のベクトル（`weekly_reports.content_vector` / `users.career_genome_vector`）を保存する。
    - Semantic Search で「疲れている」「新しいことに挑戦したい」といったニュアンス検索を可能にする。
  - **データ統合管理**
    - PostgreSQL で社員マスタ、案件マスタ、アサイン、週報、診断結果を統合管理する。
