...
@Component({
  ...
  template: `
    ...
    <p class="mt-1 text-sm text-slate-300">案件と候補者を選び、AIの「未来予測」と介入プランを確認します。</p>
      ...
      <section class="rounded-lg border border-slate-800 bg-slate-950/40 p-4 surface-panel">
        ...
        <div class="mt-2 grid gap-2">
          @for (m of store.members(); track m.id) {
            <label class="flex items-start gap-3 rounded border border-slate-800 bg-slate-900/40 px-3 py-2">
              <input
                type="checkbox"
                class="mt-1"
        ...
        <div class="mt-4">
          ...
          <div class="mt-2 min-h-12 rounded-xl border border-slate-800 bg-slate-950/30 p-3 flex flex-wrap gap-2 items-center">
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between text-xs text-slate-400">
              <span>予算消化率</span>
              <span class="text-slate-200">{{ budgetUsed() }} / {{ store.selectedProject()?.budget ?? 0 }}</span>
            </div>
            <div class="mt-2 h-2 rounded bg-slate-800 overflow-hidden">
              <div class="h-2" [style.width.%]="budgetPct()" [style.background]="budgetBarColor()"></div>
            </div>
          </div>
        </div>
      ...
      <section class="rounded-lg border border-slate-800 bg-slate-950/40 p-4 surface-panel">
        ...
        <div>
          <div class="text-sm text-slate-300">
            {{ r.project.name }} / pattern: <span class="text-slate-100 font-semibold">{{ r.pattern }}</span>
          </div>
          ...
          <div class="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <div class="rounded border border-slate-800 bg-slate-900/30 p-3">
              ...
              <div class="mt-1 text-sm">skill {{ r.metrics.skillFitPct }}% / career {{ r.metrics.careerFitPct }}%</div>
            ...
            <div class="mt-2 grid gap-2 sm:grid-cols-2">
              <div class="rounded border border-slate-800 bg-slate-900/30 p-3 text-sm">
                <div class="flex items-center justify-between">
                  ...
                  <span [class.text-emerald-300]="r.agents.pm.vote === 'ok'" [class.text-rose-300]="r.agents.pm.vote === 'ng'"
              ...
              <div class="rounded border border-slate-800 bg-slate-900/30 p-3 text-sm">
                <div class="flex items-center justify-between">
                  ...
                  <span [class.text-emerald-300]="r.agents.hr.vote === 'ok'" [class.text-rose-300]="r.agents.hr.vote === 'ng'"
          ...
          <div class="text-sm text-slate-400">案件とメンバーを選択して「AI自動編成」を押すと結果が表示されます。</div>
        }
      </section>
    </div>
    ...
    <div class="fixed inset-0 z-50">
      ...
      >
        ...
        <div class="relative h-full flex flex-col">
          <div class="h-14 shrink-0 border-b border-slate-800/80 bg-slate-950/60 backdrop-blur px-5 flex items-center justify-between">
            ...
            <button type="button" class="text-slate-300 hover:text-white text-2xl leading-none" (click)="closeOverlay()">×</button>
          </div>
          ...
          <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
            <div class="border-r border-slate-800/80 overflow-hidden flex flex-col">
              <div class="p-5 border-b border-slate-800/80 bg-white/5">
                <div class="text-[11px] text-slate-400 font-bold uppercase tracking-wider">{{ overlayKpiLabel() }}</div>
                <div class="mt-1 text-4xl font-extrabold tracking-tight" [class.text-rose-200]="overlayMode() === 'alert'" [class.text-emerald-200]="overlayMode() !== 'alert'">
                  {{ overlayKpiVal() }}
                </div>
                <div class="mt-1 text-sm text-slate-300">{{ overlayKpiDesc() }}</div>
              </div>

              <div class="px-5 py-3 text-[11px] text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800/80">Agent Log</div>
                ...
                @if (store.simulationResult(); as r) {
                  <div class="mt-3 grid gap-3 md:grid-cols-3">
                    @for (p of r.plans; track p.id) {
                        ...
                        <div class="mt-2 text-xs text-slate-300">pros: {{ p.pros.join(' / ') }}</div>
                        <div class="mt-1 text-xs text-slate-300">cons: {{ p.cons.join(' / ') }}</div>
                      </button>
                    }
                  </div>
                } @else {
                  <div class="mt-3 text-sm text-slate-400">先にシミュレーションを実行してください。</div>
                }
              </div>
...
export class SimulatorPage implements OnDestroy {
  ...
  protected readonly budgetBarColor = computed(() => (this.budgetPct() >= 100 ? '#f43f5e' : '#06b6d4'));
  ...
  private async runDemo(mode: 'alert' | 'manual'): Promise<void> {
    const projectId = this.store.projects().some((x) => x.id === 'ec') ? 'ec' : this.store.projects()[0]?.id;
  ...
  protected openOverlay(mode: 'alert' | 'manual'): void {
    ...
    if (r) {
      ...
      const script: { agent: string; tone: 'pm' | 'hr' | 'risk' | 'gunshi'; text: string }[] = r.meetingLog?.length
        ? [
        ...
        : [
            { agent: 'System', tone: 'gunshi', text: `対象: ${r.project.name}` },
            { agent: 'PM', tone: 'pm', text: `SKILL_FIT=${r.metrics.skillFitPct}% / BUDGET=${r.metrics.budgetPct}%` },
            { agent: 'HR', tone: 'hr', text: `${r.agents.hr.vote === 'ng' ? '燃え尽き/負荷' : '成長機会'} の観点で ${r.agents.hr.vote}` },
            { agent: 'RISK', tone: 'risk', text: `INCIDENT_RISK=${r.metrics.riskPct}% / pattern=${r.pattern}` },
            { agent: 'Gunshi', tone: 'gunshi', text: `結論: Plan ${r.agents.gunshi.recommend}（${r.agents.gunshi.note}）` },
          ];
      this.playLog(script);
    }
  }
